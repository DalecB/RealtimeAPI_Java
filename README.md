...
- `/internal/streams/status`의 `pendingEntries`는 릴레이 컨슈머 그룹(`audit-relay`)이 읽었지만 XACK하지 않은 건수(XPENDING)입니다. 아직 읽지 않은 건수는 포함하지 않습니다. `streamLength`는 이미 처리된 항목까지 포함한 감사 스트림의 전체 길이(XLEN)입니다.
...
| Kafka 감사 로그 왕복 | 이벤트 처리의 Lua XADD → 릴레이 1회 → Kafka 컨슈머 → PostgreSQL `audit_events` 적재를 실제 컨테이너로 연결하고, `eventId`·`userId`·`delta`·`apiKeyId` 등 원본 필드와 릴레이 처리 후 Redis PEL 0건을 검증 | `KafkaAuditRoundTripTest` |
...
| app-2 강제 종료 | 45초 뒤 app-1 = `0, 1, 2` | app-1이 파티션 2를 다시 할당받아 다음 이벤트를 저장했고 모든 파티션의 lag이 0으로 복귀 |
...
현재 릴레이 운용 범위는 **단일 인스턴스 + 고정 컨슈머 이름**이다. 중단된 릴레이가 남긴 PEL을 다른 릴레이가 `XAUTOCLAIM`으로 회수하는 기능은 [SPIKE-001의 후속 TODO](docs/SPIKE-001-kafka-migration-path.md#현재-운용-경계와-후속-todo-2026-08-10-결정)로 둔다.

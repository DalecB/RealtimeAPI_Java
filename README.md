...
현재 릴레이 운용 범위는 **단일 인스턴스 + 고정 컨슈머 이름**입니다. 중단된 릴레이가 남긴 미처리 메시지를 다른 릴레이가 `XAUTOCLAIM`으로 인계받는 기능은 [SPIKE-001의 Phase 3 예정 범위](docs/SPIKE-001-kafka-migration-path.md#현재-운용-경계와-phase-3-예정-범위)로 분리했습니다.
...
- 공식 측정은 `REDIS_MAXMEMORY=256mb docker compose up ...`로 환경을 명시해 실행합니다.
